// O'zbekcha (Lotin) — Standart til
const Map<String, String> uzStrings = {
  // ═══ Splash ═══
  'welcome': 'Xush kelibsiz!',
  'welcomeDescription':
      'Bu ilova "Ahmad Xodiy Maqsudiy — Muallimi Soniy" kitobini '
          'o\'qish va talaffuzni eshitish uchun mo\'ljallangan.',
  'wuduReminder':
      'Bu kitob Qur\'on harflarini o\'rgatadi. '
          'Iltimos, tahoratli holda foydalaning.',
  'offlineNote':
      'Ilova offline ishlaydi. Internet faqat yangi kontent '
          'yuklash uchun kerak.',
  'continueButton': 'Davom etish',

  // ═══ Home ═══
  'author': 'Muallif: Ahmad Xodiy Maqsudiy',
  'secondTeacher': 'Ikkinchi Muallim',
  'pages': '{count} sahifa',
  'recitedBy': 'O\'qidi: Jahongir qori Nematov',
  'authorLabel': 'Muallif:',
  'recitedByLabel': 'O\'qidi:',
  'foreword': 'Muqaddima',
  'forewordBody': 'Qo\'lingizdagi ushbu kitobcha va uning muallifiga chin ma\'noda baxtli taqdir nasib etgan. 1868-yilning 26-sentyabrida Qozon uyezdining Toshsuv qishlog\'ida tug\'ilib, Qozondagi "Qosimiya" madrasasida ta\'lim olgan Ahmad Hodiy Maqsudiy rahmatullohi alayhi hali ancha yoshligida ushbu qo\'llanmani tuzar ekan, u asrlar osha avloddan-avlodga Alloh taoloning Kalomi asosida arab alifbosida xat-savod o\'rgatish bilan birga dastlabki qur\'oniy saboq berishda davom etishini Parvardigori olamdan so\'ragani aniq. Hozirgi paytda bu sohada ko\'plab biri-biridan qiziqarli, salmoqli darslik va qo\'llanmalar yaratilganiga qaramasdan, muallifning boshqa asarlari, jumladan, "Ibodati islomiya" singari bu mo\'jaz kitobcha ham sodda, o\'zlashtirishga oson va dilga yaqinligi bilan hanuzgacha ko\'plab musulmon diyorlarida ilm toliblarini o\'ziga tortib kelayotgani, albatta, bu duoning ijobatidir. O\'zbekiston Respublikasi Fanlar akademiyasi Sharqshunoslik instituti ko\'lyozmalar xazinasida saqlanayotgan hujjatlarga va boshqa manbalarga asosan, kitobchaning Markaziy Osiyo hududida tarqalish tarixi quyidagicha kechganini taxmin qilish mumkin. 1902-yilda Toshkentda rus-tuzem maktablari o\'zbek sinflari uchun Saidrasul Saidazizov (1866-1933) "Ustodi avval" ("Birinchi ustoz") qo\'llanmasi nashrdan chiqadi. Bu Urta Osiyoda tovush usuli (usuli savtiya)da tuzilgan birinchi darslik bo\'lib, oktyabr inqilobiga qadar rus-tuzem maktablaridagina emas, yangi usuldagi maktablarda ham o\'zbek tilidan asosiy savod chiqarish kitobi sifatida qo\'llangan. O\'sha davrda Rossiyada Ahmad Hodiy Maqsudiyning ruscha-tatarcha "Muallimi avval" qo\'llanmasi keng shuhrat qozongan edi. Kitobchaning bizning yurtimizda nashr etilgan keyingi barcha nashrlariga asos bo\'lib xizmat qilgan nusxasi sarvarg\'ida buni tasdiqlovchi rasmiy ma\'lumotlar keltiriladi. Ushbu alifbo 1913-yil 9-aprelda muayyan raqamli hujjat bilan tatar va rus-tatar maktablarida sinfda foydalanishga kiritilgani qayd etilgach, yana bunday ta\'kidlanadi: "Un to\'rtinchi nashri. Birinchi nashriga 1892-yil 28-yanvarda Petrograd senzurasi ruxsat bergan. "Umid" shirkati matbaasi. Qozon, 1917 y.". Bundan kelib chiqadiki, birinchi nashriga ruxsat berilgach, kitobcha yigirma yildan ko\'proq muddat mobaynida norasmiy ravishda xalq ta\'limi sohasida obru qozonib ulgurgach, davlat unga maktablar uchun rasmiy qo\'llanma maqomini berishga majbur bo\'lgan. Shu davrdan e\'tiboran kitobcha Rossiya imperiyasi, keyinchalik sobiq Sovet Ittifoqi hududidagi musulmon o\'lkalarda avval lotin, so\'ngra kirill alifbosi muomalaga kiritilganiga qadar shu vazifani bajarib keldi. Kitobcha 1917-yilga qadar o\'n to\'rt marta, keyingi davrlarda yana necha o\'nlab bora nashr etilganining o\'zi buning yorqin isbotidir. Jumladan, 1913-yili Toshkentda ham faqat noshir Ali Asg\'ar (Kalinin) ismi ko\'rsatilgan "Muallimi soniy" kitobchasi paydo bo\'ladiki, u mazmunan aynan Maqsudiyning biz fikr yuritayotgan qo\'llanmasiga muvofiq kelardi. Albatta, qo\'llanmaning keyingi nashrlari muallif ismi sharifi bilan qayta-qayta chiqib turdi. O\'shandan buyon u faqat bizdagina emas, balki boshqa ko\'plab mamlakatlarda avlodlarga tengsiz beminnat muallimlik qilib keladi. Shularni hisobga olgan holda, mazkur qo\'llanmani holicha, ya\'ni nussa ko\'chirilaverishidan xiralashib qolgan xatini tiniqlashtirgan va boshqa juz\'iy texnik xatolarni tuzatgan holda, o\'quvchilarga taqdim etishga qaror qilindi. Albatta, bugungi kunda Madina bosma mushaflar keng ommalashgani bois dastalab ushbu kitobchani ham ularga monand o\'zgartirish rejasi ham yo\'q emasdi. Birok, bir jihati, uzoq davr davomida xalqlarga qadrdon bo\'lib, yaxshi xizmat kilib kelayotgan kitobchani asl xolicha saqlab kolish uni yaratgan va shu asosda ilm maydoniga qadam qo\'ygan ajdodlarimiz xotirasiga hurmat va minnatdorlik belgisi bo\'lib tuyuldi. Shunga ko\'ra, qo\'llanmaning asl ruhiyatini saqlab qolish maqsadidan kelib chiqib, kitobga deyarli o\'zgarish kiritilmadi. Faqatgina suralarning xati diyorimizda Madina bosma mushaflar ommalashgani bois harflar ko\'zga moslashishi oson bo\'lishi uchun Madina bosma xatida, harakatlari esa Qozon bosma holatida qoldi. Shuningdek, asosiy maqsad o\'quvchi arab xatini shunchaki o\'qishnigina emas, balki boshdanok har bir harfni maxraji (joy-joyidan) chiqarishni yaxshi o\'rganish orqali Qur\'on suralari qiroatiga go\'zal talaffuz bilan kirishuvini ko\'zlab, kitobga ovoz tushirilgan disk ham ilova qilinmoqda. Talaba darsni o\'zlashtirish jarayonida diskdagi ovozga diqqat qilgan holda, harflarning talaffuz sifati hamda fatha, kasra, zammadan iborat harakatlarning ado etilishiga e\'tibor qaratmog\'i lozim. Chunki Qur\'on qiroatida aksar xatolar (zod) o\'rniga (30) tovushi, (sod) o\'rniga (sin) yoki (se) tovushiga o\'xshatib, maxrajidan boshqa joydan chiqarilishi; jarangli (be) jarangsiz (pe) tovushiga o\'xshatib talaffuz etilishi; sukunli "mim", "nun" va "lom" harflarining qalqala bo\'lib tebranib qolishi kabi sifatlar va harakatlarda lablarning keragicha harakatlanmay, ixtilos (harakatning 2/3 qismi yo\'qolib, 1/3 qismi talaffuzda namoyon) bo\'lishi kabi holatlar yuz berishida kuzatiladi. Shu jihatlarga e\'tibor qaratilsa, ushbu ilovali qo\'llanma Qur\'oni karimni to\'g\'ri o\'qishni o\'rganishda yana bir muhim vosita bo\'lib xizmat qiladi, degan umiddamiz. Garchi Qur\'oni karim kalimalaridan boshqa so\'zlarni talaffuz qilishda tajvid qoidalariga rioya qilish vojib bo\'lmasa-da, maqsad Qur\'oni karimni to\'g\'ri o\'qishga odatlantirish bo\'lgani uchun imon kalimalari ham tajvid asosida o\'qib ko\'rsatildi.',
  'startReading': 'O\'qishni boshlash',
  'feedback': 'Fikr bildirish',
  'lastReadPage': 'So\'nggi o\'qilgan sahifa',
  'pageN': 'Sahifa {n}',
  'continueReading': 'Davom ettirish',

  // ═══ Settings ═══
  'settings': 'Sozlamalar',
  'theme': 'Mavzu',
  'light': 'Kunduzgi',
  'dark': 'Tungi',
  'system': 'Tizim',
  'language': 'Til',
  'aboutApp': 'Ilova haqida',
  'privacyPolicy': 'Maxfiylik siyosati',
  'termsOfUse': 'Foydalanish shartlari',
  'about': 'Dastur haqida',

  // ═══ Feedback ═══
  'feedbackTitle': 'Fikr bildirish',
  'feedbackSubtitle': 'Fikr va takliflarni qabul qilamiz',
  'fillAllFields': 'Barcha maydonlarni to\'ldiring',
  'yourName': 'Ismingiz',
  'nameRequired': 'Ism kiritilmagan',
  'phoneNumber': 'Telefon raqam',
  'phoneRequired': 'Telefon kiritilmagan',
  'phoneInvalid': 'Noto\'g\'ri format: +998XXXXXXXXX',
  'type': 'Turi',
  'suggestion': '📝 Taklif',
  'bugReport': '🐛 Xatolik',
  'details': 'Tafsilotlar',
  'minChars': 'Kamida 10 belgi kiriting',
  'submit': 'Yuborish',
  'thanks': 'Rahmat!',
  'feedbackSent': 'Fikringiz muvaffaqiyatli yuborildi.',
  'backToHome': 'Asosiy sahifaga',
  'errorOccurred': 'Xatolik yuz berdi. Qayta urinib ko\'ring.',

  // ═══ TOC ═══
  'tableOfContents': 'Mundarija',
  'noToc': 'Mundarija hali qo\'shilmagan',
  'retry': 'Qayta urinish',

  // ═══ Reader ═══
  'pageOf': 'Sahifa {current} / {total}',
  'pageNotFound': 'Sahifa topilmadi',
  'imageLoadFailed': 'Rasm yuklanmadi',
  'error': 'Xatolik',

  // ═══ Copyright ═══
  'muslimBoardUz': 'O\'zbekiston Musulmonlar Idorasi',
  'allRightsReserved': '© 2014 — Barcha huquqlar himoyalangan',

  // ═══ Legal — Privacy ═══
  'privacyTitle': '🔒 Maxfiylik siyosati',
  'privacyEffectiveDate': 'Kuchga kirgan: 2026-yil, 16-fevral',
  'privacyIntroTitle': '1. Kirish',
  'privacyIntro1':
      'Ushbu Maxfiylik siyosati "MYSTAR" MChJ tomonidan ishlab chiqilgan "Muallimi Soniy" mobil ilovasi foydalanuvchilarining shaxsiy ma\'lumotlarini to\'plash, saqlash, qayta ishlash va himoya qilish tartibini belgilaydi.',
  'privacyIntro2':
      'Ilovadan foydalanishni boshlash orqali Siz ushbu Maxfiylik siyosati shartlariga rozilik bildirasiz.',
  'privacyDataTitle': '2. Biz to\'playdigan ma\'lumotlar',
  'privacyVoluntary': '2.1. Ixtiyoriy ma\'lumotlar',
  'privacyVoluntaryDesc':
      'Ilova faqat "Fikr bildirish" bo\'limi orqali quyidagi ma\'lumotlarni qabul qiladi:',
  'privacyDataType': 'Ma\'lumot turi',
  'privacyDataPurpose': 'Maqsad',
  'privacyDataName': 'Ism',
  'privacyDataNamePurpose': 'Murojaat egasini aniqlash',
  'privacyDataPhone': 'Telefon raqam',
  'privacyDataPhonePurpose': 'Aloqa o\'rnatish',
  'privacyDataFeedback': 'Fikr matni',
  'privacyDataFeedbackPurpose': 'Ilovani yaxshilash',
  'privacyImportantNote':
      '⚠️ Muhim: Ushbu ma\'lumotlar faqat foydalanuvchi "Fikr bildirish" formasini ixtiyoriy ravishda to\'ldirib, "Yuborish" tugmasini bosgan taqdirdagina to\'planadi. Ilova avtomatik ravishda hech qanday shaxsiy ma\'lumotni to\'plamaydi.',
  'privacyLocalStorage': '2.2. Mahalliy saqlash',
  'privacyLocalStorageDesc':
      'Ilova quyidagi texnik ma\'lumotlarni faqat qurilmada mahalliy saqlaydi va serverga yubormaydi:',
  'privacyLocalItem1': 'Tanlangan til sozlamasi',
  'privacyLocalItem2': 'Tanlangan mavzu (kunduzgi/tungi rejim)',
  'privacyLocalItem3': 'Oxirgi o\'qilgan sahifa raqami',
  'privacyNotCollected': '2.3. Biz to\'plaMAYDIGAN ma\'lumotlar',
  'privacyNotGps': 'GPS joylashuv ma\'lumotlari',
  'privacyNotContacts': 'Kontaktlar, suratlar yoki qurilmadagi fayllar',
  'privacyNotDevice': 'Qurilma identifikatorlari (IMEI, MAC)',
  'privacyNotAds': 'Reklama identifikatorlari',
  'privacyNotFinance': 'Moliyaviy yoki to\'lov ma\'lumotlari',
  'privacyNotBiometric': 'Biometrik va sog\'liq ma\'lumotlari',
  'privacyUsageTitle': '3. Ma\'lumotlardan foydalanish',
  'privacyUsageDesc':
      '"Fikr bildirish" orqali olingan ma\'lumotlar faqat quyidagi maqsadlarda ishlatiladi:',
  'privacyUsage1': 'Foydalanuvchi murojaatlariga javob berish',
  'privacyUsage2': 'Ilovadagi xatolarni tuzatish',
  'privacyUsage3': 'Ilova sifatini oshirish',
  'privacyUsageNoMarketing':
      'Ma\'lumotlar marketing, reklama yoki profillash maqsadlarida ishlatilmaydi.',
  'privacyThirdPartyTitle': '4. Uchinchi shaxslarga berish',
  'privacyThirdParty1':
      'Biz foydalanuvchilarning shaxsiy ma\'lumotlarini uchinchi shaxslarga sotmaymiz, ijaraga bermaymiz yoki tijorat maqsadlarida bermaymiz.',
  'privacyThirdParty2':
      'Ma\'lumotlar faqat O\'zbekiston Respublikasi qonunchiligiga muvofiq vakolatli davlat organlarining yozma talabi yoki foydalanuvchining roziligi bilan berilishi mumkin.',
  'privacySecurityTitle': '5. Saqlash va himoya',
  'privacySecurity1': 'Barcha ma\'lumotlar HTTPS (TLS/SSL) orqali shifrlangan',
  'privacySecurity2': 'Server ma\'lumotlari xavfsiz muhitda saqlanadi',
  'privacySecurity3': 'Ma\'lumotlarga faqat vakolatli xodimlar kira oladi',
  'privacySecurity4': 'Ma\'lumotlar 90 kun ichida o\'chiriladi',
  'privacyRightsTitle': '6. Foydalanuvchi huquqlari',
  'privacyRight1': 'Ma\'lumotlarni so\'rash huquqi',
  'privacyRight2': 'Ma\'lumotlarni o\'chirish huquqi',
  'privacyRight3': 'Ma\'lumotlarni tuzatish huquqi',
  'privacyRight4': 'Rozilikni bekor qilish huquqi',
  'privacyRightsContact':
      'Murojaatlar uchun: shohruxbekaralov@gmail.com\nJavob muddati: 30 kun',
  'privacyChildrenTitle': '7. Bolalar maxfiyligi',
  'privacyChildrenDesc':
      'Ilova ta\'limiy xarakterga ega. Biz 13 yoshdan kichik bolalardan ataylab shaxsiy ma\'lumot to\'plamaymiz.',
  'privacyThirdServicesTitle': '8. Uchinchi tomon xizmatlari',
  'privacyThirdServicesDesc':
      'Ilova uchinchi tomon analitika, reklama yoki kuzatish xizmatlaridan foydalanmaydi.',
  'privacyLawTitle': '9. Amaldagi qonunchilik',
  'privacyLaw1':
      'O\'zR "Shaxsga doir ma\'lumotlar to\'g\'risida"gi Qonuni (O\'RQ-547)',
  'privacyLaw2': 'O\'zR "Axborot erkinligi" Qonuni',
  'privacyLaw3': 'Google Play Developer Distribution Agreement',
  'privacyLaw4': 'Apple Developer Program License Agreement',
  'privacyContactTitle': '10. Bog\'lanish',
  'contactCompany': 'MYSTAR MChJ',
  'contactEmail': 'shohruxbekaralov@gmail.com',
  'contactApp': 'Muallimi Soniy',
  'privacyFooter': '© 2026 MYSTAR MChJ. Barcha huquqlar himoyalangan.',

  // ═══ Legal — Terms ═══
  'termsTitle': '📋 Foydalanish shartlari',
  'termsEffectiveDate': 'Kuchga kirgan: 2026-yil, 16-fevral',
  'termsGeneralTitle': '1. Umumiy qoidalar',
  'termsGeneral1':
      'Ushbu Shartlar "MYSTAR" MChJ tomonidan ishlab chiqilgan "Muallimi Soniy" mobil ilovasidan foydalanish tartib va qoidalarini belgilaydi.',
  'termsGeneral2':
      'Ilovani o\'rnatish yoki foydalanish orqali Siz ushbu Shartlarga rozilik bildirasiz.',
  'termsDescTitle': '2. Ilova tavsifi',
  'termsDesc':
      '"Muallimi Soniy" — Ahmad Xodiy Maqsudiy tomonidan yozilgan kitobning raqamli versiyasi bo\'lib, arab alifbosini interaktiv o\'rganish uchun mo\'ljallangan bepul ta\'limiy ilovadir.',
  'termsLegalBasis':
      '📖 Huquqiy asos: O\'zbekiston Respublikasi Vazirlar Mahkamasi huzuridagi Din ishlari bo\'yicha qo\'mitaning 2014-yil 10-noyabrdagi 3438-son xulosasi asosida nashr etilgan "Muallimi soniy" kitobi va MP3 diskidan foydalanilgan.',
  'termsRightsTitle': '3. Berilgan huquqlar',
  'termsRight1': 'Ilovani bepul yuklab olish va o\'rnatish',
  'termsRight2': 'Barcha funksiyalardan cheklanmagan foydalanish',
  'termsRight3':
      'Kontentni shaxsiy ta\'limiy maqsadlarda o\'qish va tinglash',
  'termsProhibitedTitle': '4. Taqiqlangan harakatlar',
  'termsProhibited1':
      'Kontentni tijorat maqsadlarida foydalanish yoki sotish',
  'termsProhibited2': 'Ilovani dekompile yoki reverse-engineering qilish',
  'termsProhibited3': 'Ilova xavfsizlik tizimlarini chetlab o\'tish',
  'termsProhibited4': 'Ilovadan noqonuniy maqsadlarda foydalanish',
  'termsProhibited5': 'Ilova serverlariga zarar yetkazish',
  'termsLicenseTitle': '5. Dasturiy kod litsenziyasi',
  'termsLicenseDesc':
      'Dasturiy kod Creative Commons Attribution-NonCommercial 4.0 International (CC BY-NC 4.0) litsenziyasi ostida tarqatiladi.',
  'termsLicense1': '✅ Kodni ko\'rish, o\'rganish va o\'zgartirish mumkin',
  'termsLicense2': '✅ Notijorat maqsadlarda tarqatish mumkin',
  'termsLicense3': '❌ Tijorat maqsadida foydalanish TAQIQLANADI',
  'termsLicense4':
      '❌ Sotish yoki pulli xizmat sifatida taqdim etish TAQIQLANADI',
  'termsLiabilityTitle': '6. Javobgarlik cheklovi',
  'termsLiabilityDesc':
      'Ilova "MAVJUD HOLDA" (AS IS) taqdim etiladi. Kompaniya ilovaning uzluksiz yoki xatosiz ishlashini kafolatlamaydi.',
  'termsInternetTitle': '7. Internet ulanishi',
  'termsInternetDesc':
      'Ilova asosan oflayn rejimda ishlaydi. Internet faqat quyidagi hollarda talab qilinadi:',
  'termsInternet1': 'Birinchi ishga tushirish va kontent yuklash',
  'termsInternet2': 'Yangilanishlarni yuklab olish',
  'termsInternet3': '"Fikr bildirish" formasini yuborish',
  'termsIPTitle': '8. Intellektual mulk',
  'termsIP1':
      'Asl kitob matni Ahmad Xodiy Maqsudiy qalamiga mansub. VazMahkama huzuridagi Din ishlari qo\'mitasining 2014-yil 10-noyabrdagi 3438-son xulosasi asosida nashr etilgan.',
  'termsIP2': 'Audio yozuvlar: Jahongir qori Nematov.',
  'termsDisputeTitle': '9. Nizolarni hal qilish',
  'termsDisputeDesc':
      'Barcha nizolar avval muzokaralar yo\'li bilan, muzokaralar natija bermaganida O\'zbekiston Respublikasi qonunchiligiga muvofiq hal qilinadi.',
  'termsContactTitle': '10. Bog\'lanish',
  'termsFooter': '© 2026 MYSTAR MChJ. Barcha huquqlar himoyalangan.',

  // ═══ Legal — About ═══
  'aboutTitle': '📖 Dastur haqida',
  'aboutSubtitle': 'Muallimi Soniy — Ikkinchi Muallim',
  'aboutAppName': 'Muallimi Soniy',
  'aboutVersion': '1.0.0',
  'aboutDeveloper': 'MYSTAR MChJ (CodingTech.uz)',
  'aboutCategory': 'Kategoriya',
  'aboutCategoryValue': 'Ta\'lim (Education)',
  'aboutPrice': 'Narx',
  'aboutPriceValue': 'Bepul',
  'aboutLicense': 'CC BY-NC 4.0 (Open Source)',
  'aboutDescTitle': '1. Ilova haqida',
  'aboutDesc':
      '"Muallimi Soniy" (المعلم الثاني — Ikkinchi Muallim) — mashhur islom olimi Ahmad Xodiy Maqsudiy tomonidan yozilgan arab alifbosini o\'rgatishga bag\'ishlangan klassik darslikning zamonaviy raqamli versiyasidir.',
  'aboutLegalBasis':
      '📜 Huquqiy asos: O\'zbekiston Respublikasi Vazirlar Mahkamasi huzuridagi Din ishlari bo\'yicha qo\'mitaning 2014-yil 10-noyabrdagi 3438-son xulosasi asosida nashr etilgan "Muallimi soniy" kitobi va MP3 diskidan foydalanilgan.',
  'aboutAuthorTitle': '2. Muallif haqida',
  'aboutAuthorDesc':
      'Ahmad Xodiy Maqsudiy (1868–1941) — tatar-boshqird pedagogi, yozuvchi va islom olimi. "Muallimi Soniy" uning eng mashhur asarlaridan bo\'lib, arab alifbosini bosqichma-bosqich o\'rgatish uchun mo\'ljallangan.',
  'aboutAudioTitle': '3. Audio',
  'aboutAudioDesc':
      'Barcha sahifalarning ovozli talaffuzini Jahongir qori Nematov o\'qigan. 16 sahifa, MP3 formatda.',
  'aboutFeaturesTitle': '4. Imkoniyatlar',
  'aboutFeature1': '📚 16 sahifa raqamlashtirilgan kitob',
  'aboutFeature2': '🔊 Professional ovozli talaffuz',
  'aboutFeature3': '🌙 Kunduzgi va tungi rejim',
  'aboutFeature4': '🌐 5 tilda interfeys',
  'aboutFeature5': '📱 Oflayn rejim',
  'aboutFeature6': '💬 Fikr bildirish',
  'aboutFeature7': '🆓 Butunlay bepul',
  'aboutTechTitle': '5. Texnologiyalar',
  'aboutOpenSourceTitle': '6. Ochiq manba',
  'aboutOpenSource1':
      'Dasturiy kod CC BY-NC 4.0 litsenziyasi ostida ochiq (open source).',
  'aboutOpenSource2': 'GitHub: github.com/IsmoilovHabibulloh/muallimi-soniy',
  'aboutPublishTitle': '7. Nashr ma\'lumotlari',
  'aboutOriginalAuthor': 'Ahmad Xodiy Maqsudiy',
  'aboutPublishBasis': 'VazMahkama, 3438-son, 2014',
  'aboutAudio': 'Jahongir qori Nematov',
  'aboutDigitization': 'MYSTAR MChJ',
  'aboutPublishYear': '2026',
  'aboutThanksTitle': '8. Tashakkur',
  'aboutThanks1': 'O\'zbekiston Musulmonlar Idorasi',
  'aboutThanks2': 'Din ishlari bo\'yicha qo\'mita (3438-son xulosa)',
  'aboutThanks3': 'Jahongir qori Nematov — audio yozuvlar',
  'aboutThanks4': 'Ahmad Xodiy Maqsudiy rahmatullohi — asar muallifi',
  'aboutThanks5': 'Flutter va Open Source hamjamiyati',
  'aboutContactTitle': '9. Bog\'lanish',
  'aboutFooter':
      '© 2026 MYSTAR MChJ. Barcha huquqlar himoyalangan.\n© 2014 O\'zbekiston Musulmonlar Idorasi.',
};
